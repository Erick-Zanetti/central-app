...
function Main() {
  ...
  const [category, setCategory] = useState<CategoryProps>(categories ? categories[0] : null);
      ...
      <div id="tasks-area-wrapper">
        {path === "/" && (
          <header>
            ...
            <h2>
              Hoje é {getCurrentWeekDay()}, {getCurrentDay()} {getCurrentMonth()}
            </h2>
          </header>
        )}
        ...
        <div id="task-writer" onClick={() => inputRef.current.focus()}>
          <div className="left">
            ...
            <form noValidate onSubmit={handleAddTask} ref={formRef}>
              <input type="text" placeholder="Escreva uma tarefa..." ref={inputRef} />
            </form>
          </div>
